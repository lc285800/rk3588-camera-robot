...
2026-07-15完成M10和M11：离线ArUco测试与ROS节点已在RK构建。真实相机链路继续使用Fast
DDS Discovery Server；相机发布端为普通客户端，ArUco订阅端设置`ROS_SUPER_CLIENT=True`，
双方使用Reliable/Volatile图像QoS。发现建立后检测节点稳定处理15 FPS。下一次唯一任务：
M12 实现可测试PID核心。
