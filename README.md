...
三点观察建立的临时角度映射。M17已接通观测有效性检查、PID、丢失保持/回中和受限PWM，
默认禁用且必须通过ROS话题显式启用。短时手持实测确认1.00ms约0度、1.90ms约90度、
2.50ms约150度，且舵机为位置型而非360度连续旋转型；自动跟踪仍只使用1.85-1.95ms的
85-95度窄范围。目测结果不能替代升级供电后的量角器测量与连续运行验收。
