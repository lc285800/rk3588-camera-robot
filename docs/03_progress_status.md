...
项目完成**阶段P4：软件闭环**，进入**阶段P5：单轴硬件**。基础架构、三机环境、ROS摄像头、
ArUco视觉、PID、目标丢失状态机和模拟云台闭环均已通过。M14正负阶跃在Mac、WSL和RK产生
完全一致的数据：1.85秒达到90%响应，2.75秒进入2%稳定带，0%超调，稳态归一化误差约0.00217。
...
**当前唯一任务：M15 盘点PCA9685与供电。**

- 输入：PCA9685模块、独立5V舵机电源、鲁班猫I2C设备节点和安全接线要求。
- 操作：只读盘点I2C总线和候选地址，记录电源额定值、逻辑电平、共地和实际接线；不驱动舵机。
- 完成：确认I2C总线、`0x40`地址、3.3V逻辑、独立5V供电和接线照片/记录。
- 失败：硬件未到位、电压不明或地址不可见时标记受阻，不进入M16动作标定。
- 文档：更新本文件、测试计划和问题日志。
