...
围绕实测90度中心的1.85-1.95ms。M17基础视觉跟随已经完成：摄像头与SG90完成机械联动，
方向由静止目标A/B客观确定为`-1`；引导式真实闭环取得533条有效检测，平均绝对水平误差
0.0555、最终0.0341、故障0，53.2秒rosbag含2524条消息，退出后PWM和进程全部清理。现场
验证档案使用70-110度、`Kp=6`、最大5度/秒，保守代码默认不自动扩大。用户确认基础跟随可用，
响应舒适度留到M20调优。M18硬件故障注入已经完成：观测源退出后状态按保持/回中转移；USB
摄像头断开导致节点明确报错退出，重新绑定和重启后恢复15 FPS；PWM已有消费者时保持禁用，
连续启用消息只产生一次接管尝试。当前唯一实施任务切换为M19 bringup与systemd待命部署。
...
| M18 | 完成硬件故障注入 | 已完成 | 相机/PWM/节点异常安全状态记录 |
| M19 | 建立bringup与systemd | 未开始（下一任务） | 开机待命、人工启用和结构化日志 |
...
**当前执行任务：M19 bringup与systemd；M15-M16保留长期供电与量角待办。**
...
- 2026-07-21鲁班猫重新上电后，Mac和WSL均在线，RK的ROS 2、colcon、`/dev/video1`和
  ...
  因此当时M17保持进行中，等待ID 23标记入镜后的动态误差与视频验收。证据位于
...
- 输入：鲁班猫5物理Pin 32、`GPIO4_B6 / PWM13_M1`、SG90，以及满足裕量要求的板卡或舵机
  供电方案。
- 2026-07-22完成M18：观测生产进程退出后完整记录`tracking/holding/returning`，测试使用零误差，
  PWM始终为1.90ms中心；USB接口解绑使相机节点以明确`No such device`故障和状态1退出，重新
  绑定后恢复约15 FPS；外部消费者占用PWM时连续10Hz启用消息只触发一次拒绝，通道保持禁用。
  测试后相机、NVMe和以太网在线，无残留进程且PWM未导出。证据位于
  `artifacts/20260722/M18-fault-injection/`。
- 下一步操作：建立默认待命的M19 bringup和systemd单元，开机不得自动启用跟踪，明确人工
  启用入口、受限重启策略和结构化日志。
- 文档：更新本文件、测试计划和问题日志。
