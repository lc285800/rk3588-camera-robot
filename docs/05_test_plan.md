...
| T-END-01/M17真实链预检 | 2026-07-21 | 当时部分通过（后续已覆盖） | 真实相机与ArUco连续处理900帧约15 FPS、错误0；当时画面ID 23检测0，后续由引导式闭环完成动态验收 |
| T-END-01/M17安全门控 | 2026-07-21 | 通过 | 无标记时连续5帧门控在3秒超时后失败关闭，舵机未启用、运行进程无残留、PWM未导出；后续最终验收同时验证显式启用确认与正常清理 |
...
| T-END-01/M17引导式闭环 | 2026-07-21 | 通过（基础） | 81/81帧预检后显式确认启用；533条有效检测，平均绝对X误差0.0555、最终0.0341，PWM 1.8368-1.9000ms、故障0；53.2秒rosbag含2524条消息，用户确认方向正确且基础跟随基本可用 |
| T-SERVO-04/M18 PWM占用 | 2026-07-22 | 通过 | 外部导出但禁用PWM时节点拒绝接管并保持disabled；10Hz连续启用消息只触发1次尝试，无高速重试，测试通道始终enable=0 |
| T-END-02/M18观测源退出 | 2026-07-22 | 通过 | 零误差观测源退出后状态覆盖tracking、holding、returning，所有记录脉宽均为1.90ms中心，显式禁用后PWM释放 |
| T-CAM-03/M18 USB断开 | 2026-07-22 | 通过 | 解绑实际uvcvideo接口后节点明确报告VIDIOC_DQBUF/No such device并以状态1退出；重新绑定和重启后恢复约15 FPS、missed=0 |
