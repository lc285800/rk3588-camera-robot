...
### ISSUE-20260722-19：M18断流与PWM占用故障可能无限重试

- 状态：已解决。
- 环境：鲁班猫5、ROS 2 Humble、USB UVC摄像头、PWM13_M1。
- 现象：旧相机节点对不可恢复的`DQBUF/QBUF`错误只做节流日志并继续定时读取；跟踪节点收到
  连续`true`消息且PWM被占用时，会在每条消息上重新尝试获取硬件。
- 风险：设备断开后形成无意义循环，或故障状态下高速重试PWM，与T-CAM-03和T-SERVO-04不符。
- 修复：相机节点对不可恢复V4L2错误立即抛出并退出，增加默认1秒无帧看门狗；跟踪节点锁存
  启用请求，每个`false -> true`沿最多尝试获取一次。
- 回归：真实解绑`6-1:1.0`后节点以`No such device`和状态1退出，重新绑定后恢复15 FPS；外部
  占用PWM时10Hz启用消息只产生一次拒绝，通道保持禁用。观测源退出按保持、回中转移，测试后
  相机、NVMe、网线在线且PWM释放。证据位于`artifacts/20260722/M18-fault-injection/`。
...
- 状态：部分解决，方向已确认，等待可靠供电和机械量角。
- 风险：选错PWM复用、信号管脚、供电或脉宽范围可能导致无响应、板卡掉压、抖动或撞限位。
- 决策：首版单轴不采购PCA9685，使用物理Pin 32的`GPIO4_B6 / PWM13_M1`输出信号；
  SG90仍使用独立5V供电并与鲁班猫共地。
- 回归：2026-07-17无外设软件验证成功输出50 Hz、1.5 ms脉宽，Pin 142复用为PWM13。
- 硬件冒烟：2026-07-20真实SG90执行1.50/1.45/1.50/1.55/1.50ms短序列，用户确认轻微转动；
  RK未重启，摄像头、NVMe和网线保持在线，PWM已安全关闭并取消导出。
- 受限标定：单独复验确认1.45ms向左、1.50ms居中、1.55ms向右；临时软件映射使用
  85/90/95度名义值，双端硬件包各38项、工作区各82项测试通过。
- 待验证：当前板卡仅使用12V 1.5A输入且SG90暂接板载共享5V，不能证明供电裕量。升级为
  12V 3A输入或独立稳压5V舵机电源后，再完成M15，并对M16做实际机械量角和边界复验。
...
### ISSUE-20260720-16：受限PWM核心首次回归与清理边界

- 状态：已解决。
- 环境：WSL与RK ROS 2 Humble，`lubanvision_hardware`首次构建测试。
- 无效尝试：在加载ROS环境前启用`set -u`，两端均因`AMENT_TRACE_SETUP_FILES`未定义而在
  构建前退出；Mac的系统Python和工作区Python均无pytest，未在Mac临时安装依赖。
- 首轮回归：20项功能测试通过，但PEP257因`__exit__`文档字符串不是祈使语气而失败。
- 安全审计：修正文档字符串时发现，导出后若等待通道节点超时，旧实现可能不请求取消导出；
  启用写入报错时也应立即尝试关闭。
- 修复：ROS环境在nounset前加载；修正文档字符串；导出写入成功后立即记录所有权；新增导出
  超时清理测试；启用写入异常时尝试关闭。
- 回归：WSL和RK硬件包各23项测试全部通过，全工作区各67项测试0失败；RK真实PWM未导出。

### ISSUE-20260721-17：M17真实闭环视场方向与安全启动

- 状态：安全启动工具已完成，机械朝向待用户调整后复核。
- 环境：鲁班猫5、`/dev/video1`、单轴Pan云台、M17默认禁用跟踪节点。
- 现象：实拍640x480画面明显侧转并朝向桌面/柜体下方；当前场景没有ID 23标记。若画面水平轴
  与Pan机械转轴不一致，控制器虽能识别旋转后的ArUco，图像X误差却不能代表实际Pan方向。
- 风险：未经目标检测直接启动会造成无意义动作；脚本异常可能留下相机、检测或录包进程。
- 修复：新增确定性的ID 23标记和`m17_live_acceptance.sh`。脚本要求连续5帧检测才显式启用，
  使用精确进程组管理相机、检测、控制、指标和rosbag；退出时发布禁用并复核PWM，绝不接管
  已导出的通道。
- 无效尝试：首轮脚本在加载ROS环境前保持`set -u`，因Humble环境脚本读取未定义变量而提前
  退出；修正为仅在source期间关闭nounset。本轮发生在启动节点前，PWM未导出。
- 回归：无标记3秒门控超时按预期返回失败，未启用舵机，所有精确进程组退出且PWM清理通过。
- 待办：将相机安装为画面水平轴与Pan轴一致，让ID 23标记入镜后运行20秒真实动态验收。

### ISSUE-20260721-18：SG90名义角度与实际位置不一致

- 状态：已修正当前默认映射，精确量角和长期端点安全性仍待验收。
- 现象：原1.45/1.50/1.55ms只验证方向，却被临时标成85/90/95度；扩展测试中也曾把脉宽中点
  1.75ms误称为90度，实际用户观察约80度。连续序列结束后的视觉判断又造成位置混淆。
- 根因：在没有量角器的情况下把理论线性中点和命令标签误当成机械实测角度。
- 实测事实：用户手持观察确认1.00ms约0度、1.90ms约90度、2.50ms约150度；各点均停止在
  固定位置并可返回。舵机为位置型，非360度连续旋转型，未实测到180度。
- 修复：以后一次只保持一个待确认位置；代码与YAML的自动跟踪中心改为1.90ms，默认硬限位
  改为1.85-1.95ms，对应名义85-95度。扩展端点只记录为短时目测标定，不用于自动跟踪。
- 预防：角度标签必须区分“命令值”“目测值”和“量具实测值”；用户确认前不得写成正式角度。

### ISSUE-20260721-19：安装后主观方向判断矛盾

- 状态：已解决。
- 现象：移动手机的动态测试中，用户先后观察到方向可能正确和可能相反；大幅移动又导致标记
  离开画面，无法仅凭摄像头动作判断控制符号。
- 根因：手机、摄像头和图像坐标的左右参照不同，同时移动目标会改变误差，主观观察不可靠。
- 方法：舵机每轮从90度启动，手机标记固定不动；分别运行`direction=+1`和`-1`，比较同一轮
  最初10条与最后10条观测的绝对水平误差。
- 结果：`+1`从0.590恶化到0.754；`-1`从0.185收敛到0.035，降低约81%。负方向测试持续
  检测91帧、故障0，PWM从1.900增至1.924ms且结束后释放。
- 修复：硬件节点、平台YAML和验收脚本默认方向统一固定为`-1`；后续方向回归必须使用静止
  目标的误差收敛数据，不接受单独的肉眼方向描述作为结论。

### ISSUE-20260721-20：M17验收启用竞态与人工动作时机不明确

- 状态：已解决。
- 现象：一次动态测试的启用消息先匹配rosbag而非跟踪节点，整轮状态均为disabled；人工测试
  又因不知道何时移动手机而产生无效数据。手机过近且屏幕过亮时识别率仅28.7%。
- 修复：启用/禁用消息改为多次可靠发布；开始计时前订阅`/gimbal/state`并强制确认
  `enabled=true`。最终测试由模型逐步发出中央、左移、回中、右移指令；手机后移20-30厘米、
  亮度约50%，使预检恢复到81/81帧。
- 回归：最终引导式闭环533条有效检测，平均绝对误差0.0555、最终0.0341、故障0；PWM和所有
  进程清理通过。未启用的竞态轮明确作废，不纳入性能结论。
