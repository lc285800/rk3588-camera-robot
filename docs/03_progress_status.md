...
12V 3A，或改用独立稳压5V舵机电源。按用户指示，M16已在既有1.45-1.55ms范围内受限启动，
但供电复测和机械量角完成前不扩大范围。M14正负阶跃在Mac、WSL
...
**当前执行任务：M16 SG90保守标定；M15保留最终电源复测待办。**
...
- 完成：确认PWM13_M1、50Hz输出、供电额定值、共地和接线记录，并在最终供电条件下重复
  中位及小步动作且RK和主要外设保持在线。
- 失败：电压不明、无法共地或PWM输出异常时保持进行中，不扩大M16动作范围。
- 文档：更新本文件、测试计划和问题日志。
