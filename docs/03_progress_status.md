...
PWM软件预检已经通过，当前只等待用户连接独立5V舵机电源、共地、SG90和Pin 32信号线；
硬件接线确认前不进入M16。M14正负阶跃在Mac、WSL和RK产生完全一致的数据：1.85秒达到
90%响应，2.75秒进入2%稳定带，0%超调，稳态归一化误差约0.00217。
...
- 当前状态：PWM设备树复用、50Hz周期、1.5ms中位脉宽和安全关闭的软件测试已通过；等待
  用户完成独立5V电源、共地、SG90与Pin 32信号线的实际连接和核对。
- 输入：鲁班猫5物理Pin 32、`GPIO4_B6 / PWM13_M1`、SG90和独立5V舵机电源。
- 下一步操作：连接并核对独立5V舵机电源、共地、SG90和Pin 32信号线，留存实际接线记录；
  已通过的软件PWM测试无需重复作为当前阻塞项。
- 完成：确认PWM13_M1、50Hz输出、独立5V供电额定值和接线照片/记录。
