...
通过 RK3588 硬件 PWM 控制 SG90 云台，持续将目标保持在画面中心。
...
- 可测试的PID视觉伺服控制器。
- RK3588 Linux PWM驱动和SG90安全角度控制。
- 目标丢失后的保持、回中和有限扫描状态机。
- 摄像头、PWM或节点异常时停止更新舵机，禁止无限旋转或撞限位。
...
标定和M17单轴视觉跟踪节点。M17基础跟随已完成：物理方向由静止目标A/B确定为`-1`，最终
引导式闭环取得533条有效检测，平均绝对水平误差0.0555、最终0.0341、故障0，退出后PWM与
全部进程安全清理。保守默认仍使用1.85-1.95ms；经现场验证的70-110度、`Kp=6`、最大5度/秒
配置单独记录为测试档案，不自动取代安全默认。短时手持实测确认1.00ms约0度、1.90ms约
90度、2.50ms约150度，舵机为位置型而非360度连续旋转型；目测结果不能替代升级供电后的
量角器测量与连续运行验收。M18硬件故障注入已完成：观测进程退出后按保持/回中转移，USB
摄像头断开时节点明确报错退出且重连重启恢复，PWM已有消费者时每个启用沿只拒绝一次且不
接管。当前下一任务为M19 bringup与systemd待命部署。
